...
export const CURRENT_TEXT_AI_MODELS = [
  "distilbert-base-uncased-finetuned-sst-2-english",
];

// 🔥 Lightweight models
const MODELS = {
  text: CURRENT_TEXT_AI_MODELS[0],
...
// 🚀 Core HF Request
// -----------------------------
async function hfRequest(model, payload, options = {}) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), options.timeout || 80000);
...
export async function analyzeTextContent({ text }) {
  const result = await hfRequest(MODELS.text, { inputs: text });
  ...
  return {
    ...
    models: {
      aiGeneratedText: MODELS.text,
    },
    overall: {
      status: top.label === "POSITIVE" ? "likely_real" : "possible_ai",
      label: top.label,
      message: "Basic classification result",
    },
    aiGeneratedText: {
      predictions: result,
      summary: {
        aiScore: top.score || 0,
        aiPercent: (top.score || 0) * 100,
        flagged: top.score > 0.7,
      },
    },
  };
}
...
export async function summarizeTextContent({ text }) {
  const result = await hfRequest(MODELS.summarization, {
    inputs: text,
    parameters: {
      max_length: 80,
      min_length: 20,
  ...
  const summaryText = result?.[0]?.summary_text || text;
...
export async function translateTextToEnglish({ text }) {
  const result = await hfRequest(MODELS.translation, {
    inputs: text,
    parameters: {
      tgt_lang: "eng_Latn",
    },
...
export async function describeImageContent({ buffer, mimeType, attachmentName }) {
  const res = await fetch(`${HF_BASE_URL}/${MODELS.imageCaption}`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${HF_API_KEY}`,
  ...
  return {
    checkedAt: new Date().toISOString(),
    models: {
      imageDescription: MODELS.imageCaption,
    },
    descriptionText: result?.[0]?.generated_text || "",
    attachmentName,
  };
}

// -----------------------------
// 🖼️ IMAGE ANALYSIS (NSFW only)
// -----------------------------
export async function analyzeImageContent({ buffer, mimeType }) {
  const res = await fetch(`${HF_BASE_URL}/${MODELS.nsfw}`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${HF_API_KEY}`,
  ...
  return {
    ...
    overall: {
      status: top.score > 0.7 ? "high_risk" : "clear",
      label: top.label,
      message: "NSFW detection",
    },
    nsfw: {
      predictions: result,
      summary: {
...
async function checkGoogleSafeBrowsing(urls) {
  if (!GOOGLE_API_KEY) return { matches: [], configured: false };
  ...
  try {
    const res = await fetch(
      `https://safebrowsing.googleapis.com/v4/threatMatches:find?key=${GOOGLE_API_KEY}`,
      {
        ...
        body: JSON.stringify({
          client: { clientId: "securechat", clientVersion: "1.0" },
          threatInfo: {
...
export async function analyzeLinkContent({ text, links }) {
  ...
  // 🔥 NEW: reputation check
  const reputationData = await checkGoogleSafeBrowsing(links);
  ...
  for (const link of links.slice(0, 3)) {
    try {
      ...
      const predictions = Array.isArray(res) ? res : [];

      const phishingScore =
        predictions.find((p) => p.label === "LABEL_1")?.score || 0;

      const isFlagged = reputationData.matches.some(
        (m) => m.threat?.url === link
      );

      const finalScore = isFlagged ? 1 : phishingScore;

      const summary = {
        phishingPercent: Math.round(finalScore * 100),
        safePercent: Math.round((1 - finalScore) * 100),
        status: finalScore > 0.6 ? "high_risk" : "clear",
  ...
  return {
    checkedAt: new Date().toISOString(),
    models: {
      phishing: MODELS.phishing,
      reputation: "Google Safe Browsing",
    },

    // 🔥 REQUIRED FOR FRONTEND
    reputation: {
      configured: reputationData.configured,
      flaggedCount: reputationData.matches.length,
      matches: reputationData.matches.map((m) => ({
        url: m.threat.url,
        threatType: m.threatType,
        platformType: m.platformType,
      })),
      status: reputationData.matches.length ? "high_risk" : "clear",
      label: reputationData.matches.length ? "Flagged links detected" : "No threats",
    },

    overall: {
      status: reputationData.matches.length ? "high_risk" : "clear",
      label: "Combined link analysis",
      message: "HuggingFace + Google Safe Browsing",
    },
